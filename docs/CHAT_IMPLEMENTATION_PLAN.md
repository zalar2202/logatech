...
> **Purpose:** Single reference for implementing live chat in logatech-en (panel chat + visitor chat), using FCM + polling (no Socket.io). Admin can be notified via WhatsApp/Telegram when visitors send messages so they do not need to be always online in the panel.
...
|------|----------------------|
| FCM (client) | [logatech-en] `src/lib/firebase/client.js` – `getToken`, `onForegroundMessage` |
| FCM (admin) | [logatech-en] `src/lib/firebase/admin.js` – send push |
| User FCM tokens | [logatech-en] `src/models/User.js` – `fcmTokens`, `addFcmToken`, `getActiveFcmTokens` |
| Notifications | [logatech-en] `docs/PANEL/notifications/README.md` |
