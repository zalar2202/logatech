...
export default function SmartCTA({ 
    ...
    const getIcon = () => {
        ...
        if (lowLabel.includes("talk") || lowLabel.includes("connect")) return <MessageSquare className="w-4 h-4 mr-2" />;
        if (lowLabel.includes("audit") || lowLabel.includes("review")) return <ShieldCheck className="w-4 h-4 mr-2" />;
        if (lowLabel.includes("schedule") || lowLabel.includes("book")) return <Calendar className="w-4 h-4 mr-2" />;
        if (lowLabel.includes("start") || lowLabel.includes("launch")) return <Zap className="w-4 h-4 mr-2" />;
        return null;
    };
